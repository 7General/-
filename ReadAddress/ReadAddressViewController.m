...
@interface ReadAddressViewController ()

@end

@implementation ReadAddressViewController
...
- (void)viewDidLoad {
    ...
    [btn2 addTarget:self action:@selector(valueAddress) forControlEvents:UIControlEventTouchUpInside];
...
- (void)copyAddressBook:(ABAddressBookRef)addressBook
{
    ...
    for ( int i = 0; i < numberOfPeople; i++){
        ...
        for (int k = 0; k<ABMultiValueGetCount(phone); k++)
        {
            ...
            NSLog(@"-----%@>>>%@>>>>>>%@",firstName,lastName,personPhone);
        }
