...
@interface ZZGPopoverView()<UITableViewDelegate, UITableViewDataSource>
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<ZZGPopoverAction *> *actions;
...
- (CGFloat)calculateMaxWidth {
    ...
    for (ZZGPopoverAction *action in _actions) {
        imageWidth = 0.f;
        titleLeftEdge = 0.f;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [UIView animateWithDuration:0.25f animations:^{
        ...
        if (self.popDelegate && [self.popDelegate respondsToSelector:@selector(popoverView:didSelectIndex:)]) {
            [self.popDelegate popoverView:self didSelectIndex:indexPath];
        }
